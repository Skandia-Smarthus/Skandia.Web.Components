...
function OnboardingCell(cell, email) {
    //debugger;
    //if (isLoading()) {
    //    return;
    //}
...
function showOrderDetails() {
    ...
    var x = customerHtml.replace(/{{Firstname}}/ig, obj.firstName);
    x = x.replace(/{{Lastname}}/ig, obj.lastName);
    x = x.replace(/{{Cell}}/ig, obj.cell);
    x = x.replace(/{{Email}}/ig, obj.email);
    ...
    deliveries.forEach((del, index) => {
        var x = deliveriesHtmlTemplate.replace(/{{Address}}/ig, del.address);
        x = x.replace(/{{HouseNumber}}/ig, del.houseNumber);
        x = x.replace(/{{ZipCode}}/ig, del.zipcode);
        x = x.replace(/{{Place}}/ig, del.place);

        if (index > 0) {
            x = x.replace(/{{andClass}}/ig, '');
        }
        else {
            x = x.replace(/{{andClass}}/ig, 'hidden');
        }
