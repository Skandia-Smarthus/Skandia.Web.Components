...
function checkMoveDateDays() {
    let selectedDate = new Date(document.getElementById('takoverDate').value);

    let today = new Date();

    // Legg til 16 dager på dagens dato
    let futureDate = new Date();
    futureDate.setDate(today.getDate() + 16);

    // Sjekk om valgt dato er mindre enn 16 dager frem
...
function validateMoveChecks() {
    debugger;
    var lessThan16Days = checkMoveDateDays();
    let isChecked = document.getElementById('supply-takover-noregret').checked;

    if (lessThan16Days && !isChecked)
        return false;
    else
        return true;
